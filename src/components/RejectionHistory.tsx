...
import { Search, Clock, Building2, ChevronRight, Trash2, AlertTriangle, X, FileText, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { cn } from '../lib/utils';

interface Props {
  history: RejectionDiagnosis[];
  onSelect: (result: RejectionDiagnosis) => void;
...
export const RejectionHistory: React.FC<Props> = ({ history, onSelect, onDelete, onClearAll, searchQuery, onSearchChange }) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input 
            type="text"
            placeholder="Search by bank or reason..."
            value={searchQuery}
        ...
        {showConfirmClear && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-bold text-zinc-900 mb-2">Clear Diagnosis History?</h3>
              <p className="text-zinc-500 mb-8 leading-relaxed">
                This will permanently delete all {history.length} past rejection diagnoses. This action cannot be undone.
              </p>
      ...
      <div className="space-y-3">
        {filteredHistory.length > 0 ? (
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-bold text-zinc-900">{item.bankName}</span>
                    <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest ml-2">
                      {new Date(item.timestamp).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 text-xs text-zinc-500">
                    <div className="flex items-center gap-1.5 line-clamp-1 max-w-md">
          ...
          <div className="text-center py-20 bg-zinc-100/50 rounded-3xl border border-dashed border-zinc-200">
            ...
            <p className="text-zinc-500 font-medium">No diagnoses found matching your search.</p>
          </div>
        )}
      </div>
    </div>
